...
//THIS SCRIPT IS THE ORIGINAL SCRIPT FOR THE THEME TOGGLE
/*
...
//THIS FUNCTI0N IS EXPORTED AND LOADED TO INDEX.JS (MAIN SPAGHETTI CODE SITE)
const exportedScript = () => {

    const themeElement = document.querySelector('theme-selector')
    const shadow = themeElement.shadowRoot
    const toggleMain = shadow.getElementById('main-toggle')
    
    const circle = shadow.getElementById('circle')
    const sky = shadow.getElementById('tgl-box')
    const stars = shadow.querySelectorAll('.star')
    const cloud = shadow.querySelectorAll('cloud')
    circle.style.transition = '0.4s ease-in-out'
    sky.style.transition = '0.4s ease-in-out'
    toggleMain.addEventListener('touchstart' && 'click', () => {
